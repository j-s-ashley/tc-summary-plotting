...
parser.add_argument("-t", "--tests",
  help="Test types to be plotted (3PG, 10PG, NO). If not specified, all will be plotted.", nargs="+", default = ["3PG", "10PG", "NO"])
...
if query_db: #if getting data from the database
    ...
    TPG_files, RC_files, NO_files = db.get_files()
    files = {'3PG': TPG_files,
             '10PG': RC_files,
             'NO': NO_files} #files sorted by type
...
print("\nMaking PDF...")
all_plots0 = make_one_list(TPG_plots) + make_one_list(RC_plots) + NO_plots #all plots made
